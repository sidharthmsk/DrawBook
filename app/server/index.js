...
const AUTH_TOKEN = APP_PASSWORD
    ...
    : "";
const ENABLE_TLDRAW = (process.env.ENABLE_TLDRAW || "false").toLowerCase() === "true";
...
const ENV_FILE_PATH = fs.existsSync(path.resolve(process.cwd(), ".env"))
...
app.get("/api/config", (_req, res) => {
    res.json({ enableTldraw: ENABLE_TLDRAW, storageBackend: STORAGE_BACKEND });
});
...
function writeEnvFile(filePath, existingContent, updates) {
...
app.get("/api/settings", (req, res) => {
    try {
        ...
        res.json({
            ...
            hasMinioCredentials: !!(process.env.MINIO_ACCESS_KEY &&
                process.env.MINIO_SECRET_KEY &&
                (process.env.MINIO_ENDPOINT_URL || process.env.MINIO_ENDPOINT)),
        });
    }
    ...
    try {
        ...
        for (const [camelKey, envKey] of Object.entries(ENV_KEY_MAP)) {
            if (camelKey in body) {
                ...
                if (SECRET_FIELDS.has(camelKey) && val.startsWith("****"))
                    continue;
...
function getCachedResponse(key) {
    ...
    if (entry && Date.now() - entry.time < RESPONSE_CACHE_TTL)
        return entry.data;
    return null;
}
...
app.get("/api/resolve", async (req, res) => {
    try {
        ...
        const exact = all.find((item) => !item.meta.deletedAt &&
        ...
        const partial = all.find((item) => !item.meta.deletedAt &&
...
app.get("/api/backlinks/:documentId", async (req, res) => {
    try {
        ...
        for (const item of all) {
            if (item.meta.deletedAt || item.id === documentId)
                continue;
            try {
                const doc = await storage.loadDocument(item.id);
                if (!doc)
            ...
            try {
                ...
                if (type === "markdown") {
                    ...
                    for (const col of columns) {
                        const cardLabels = (col.cardIds || []).map((cid) => {
...
function extractText(doc, type) {
    ...
    if (type === "spreadsheet") {
        ...
        for (const [, sheet] of Object.entries(sheets)) {
            ...
            if (!cellData)
                continue;
            for (const [rowIdx, row] of Object.entries(cellData)) {
                if (!row)
                    continue;
                for (const [colIdx, cell] of Object.entries(row)) {
                    const val = cell?.v;
...
app.post("/api/import/obsidian", upload.single("file"), async (req, res) => {
    try {
        ...
        const folderMap = new Map(); // zip path -> folder id
        const existingFolders = await loadFolders();
        const newFolders = [];
        // Create a root folder for the vault
        const vaultName = file.originalname.replace(/\.zip$/i, "") || "Obsidian Vault";
        ...
        for (const dirPath of sortedDirs) {
            ...
            if (!relative || folderMap.has(relative))
                continue;
        ...
        for (const entry of entries) {
            ...
            if (entryExt === ".md") {
                ...
                content = content.replace(/!\[\[([^\]|]+?)(?:\|([^\]]*))?\]\]/g, (_, target, alt) => `![${alt || target}](${target})`);
            ...
            else if ([".json", ".canvas"].includes(entryExt) &&
        ...
        invalidateResponseCache();
        console.log(`[Server] Obsidian import: ${imported} imported, ${skipped} skipped, ${newFolders.length} folders`);
        res.json({
            success: true,
            imported,
            skipped,
...
app.post("/api/templates/from-doc/:documentId", async (req, res) => {
    try {
        ...
        templates.push(template);
        await storage.saveTemplates(templates);
        console.log(`[Server] Saved doc as template: ${documentId} -> ${template.name}`);
        res.json({ template });
    }
    catch (error) {
...
function dailyNoteId(date) {
    return `${DAILY_NOTE_PREFIX}${date}`;
}
function dailyNoteTemplate(date) {
...
app.get("/api/daily-notes", async (_req, res) => {
    try {
        const all = await storage.listDocumentsWithMeta();
        const notes = all
            .filter((item) => !item.meta.deletedAt && item.id.startsWith(DAILY_NOTE_PREFIX))
            .map((item) => ({
            id: item.id,
...
export async function startServer(overrides) {
    ...
    if (overrides?.dataDir)
        process.env.DATA_DIR = overrides.dataDir;
    const port = Number(process.env.PORT) || 3000;
    await storage.init();
    console.log(`[Server] Storage backend: ${STORAGE_BACKEND}`);
    return new Promise((resolve, reject) => {
        server.listen(port, () => {
            console.log(`[Server] Drawbook server running on http://localhost:${port}`);
